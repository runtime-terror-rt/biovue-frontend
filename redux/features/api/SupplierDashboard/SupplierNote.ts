...
export interface UpsertSupplierNoteRequest {
  id?: number; // ✅ required for update, optional for create
  user_id: number;
  note: string;
}

export interface UpsertSupplierNoteResponse {
  success: boolean;
  message: string;
  note_id: number;
}
...
export const supplierNoteApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    upsertSupplierNote: builder.mutation<
      UpsertSupplierNoteResponse,
      UpsertSupplierNoteRequest
    >({
      query: (body) => ({
        url: "/trainer-notes",
        method: "POST",
        body, // { id?, user_id, note }
      }),
      invalidatesTags: (_result, _error, { user_id }) => [
        { type: "SupplierNote", id: user_id },
      ],
